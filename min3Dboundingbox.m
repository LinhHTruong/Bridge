function [rotmat,cornerpoints,volume,surface,edge_vector,edgelength] = min3Dboundingbox(x,y,z,metric,level)
...
try
    ...
    fprintf('The number and/or distribution of given points does not allow the construction of a convex hull.')
    rotmat = [];
    cornerpoints = []; % minimal boxes' cornerpoints
    volume = [];
    surface = []; % Sum of surface area
    %edgelength = 4*sum(h);
    edgelength = [];
    return;
...
verticies = [minmax(1,1),minmax(1,2),minmax(1,3);    % xmin,ymin,zmin
...
cornerpoints = verticies*inv(rotmat); % minimal boxes' cornerpoints
% Determine the edge direction
edge_vector = [cornerpoints(2,:) - cornerpoints(1,:);
			   cornerpoints(4,:) - cornerpoints(1,:);
			   cornerpoints(5,:) - cornerpoints(1,:)];

% Calculate edge length           
edgelength = vecnorm(edge_vector,2,2);

% Adjust direction according to axis
edge_vector = edge_vector./edgelength;
edge_vector_ox = sum(bsxfun(@times,edge_vector, [1.,0.,0.]),2);
[~, ox_id] = max(abs(edge_vector_ox));
edge_vector_oy = sum(bsxfun(@times,edge_vector, [0.,1.,0.]),2);
[~, oy_id] = max(abs(edge_vector_oy));
edge_vector_oz = sum(bsxfun(@times,edge_vector, [0.,0.,1.]),2);
[~, oz_id] = max(abs(edge_vector_oz));
edge_vector = edge_vector([ox_id, oy_id, oz_id],:);
edgelength = edgelength([ox_id, oy_id, oz_id]);

% Calculate Volume and surface
volume = edgelength(1)*edgelength(2)*edgelength(3);
surface = 2*(edgelength(1)*edgelength(2)+edgelength(2)*edgelength(3)+edgelength(3)*edgelength(1)); % Sum of surface area
%edgelength = 4*sum(h);
%edgelength = h;
