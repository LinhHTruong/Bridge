function [Region] = backward_forward_filtering(TREE, Cells, region_info, THRESHOLD)
...
Region = struct('cell',[], 'status', []);
% preallocate structure
for region_id = 1:no_region
    Region(region_id).status = [];
    Region(region_id).cell.id = [];
...
for i = 1:no_region
    ...
    for j = 1:size(region_cell_ids, 1)
        ...
        region_current_cell_id = region_cell_ids(j,:);
        
        % Searching neighbour cells
        neighbour_cell_ids = Query_Neighbour_Cells(TREE, region_current_cell_id(1),region_info(:,1));
        ...
        if isempty(out_region_cell_info) %Isolate region or on the boundary and no attached to any region 
            ...
            mask = ismember(Cells.cell_ids(:,[1,2]), region_current_cell_id, 'rows');
            region_current_cell_peak_ptc_ids = Cells.peak_info(mask).ptc_ids;
            
 
            if size(in_region_cell_info,1) == 8 %Interior cell -> no filtering
                % Cell peak features
                region_current_cell_peak_features = Cells.peak_info(mask).peaks_features(1:end-1);
            else
                % Points in the current 
                region_current_cell_peak_ptc_xyz = TREE.pts(region_current_cell_peak_ptc_ids,1:3);
                ...
                mask = ismember(Cells.cell_ids(:,[1,2]), in_region_cell_info(:,[1,2]), 'rows');
                in_region_neighbour_cell_peak_ptc_ids = vertcat(Cells.peak_info(mask).ptc_ids);
                in_region_neighbour_cell_peak_ptc_xyz = TREE.pts(in_region_neighbour_cell_peak_ptc_ids,1:3);
                in_region_neighbour_cell_peak_center = mean(in_region_neighbour_cell_peak_ptc_xyz,1);
                in_region_neighbour_cell_peak_normal = eigenspace(in_region_neighbour_cell_peak_ptc_xyz,1);
                
                % Compute distance
                dist_region_ptc = abs(dist_3Dpoints_3Dplane(region_current_cell_peak_ptc_xyz, [in_region_neighbour_cell_peak_center, in_region_neighbour_cell_peak_normal]));
                
                % Filter inlier
                mask = dist_region_ptc <= THRESHOLD.dev_plane;
                region_current_cell_peak_ptc_ids = region_current_cell_peak_ptc_ids(mask);
                if numel(region_current_cell_peak_ptc_ids) >= THRESHOLD.min_num_pts
                    region_current_cell_peak_ptc_xyz = region_current_cell_peak_ptc_xyz(mask,:);
                    region_current_cell_peak_features = [mean(region_current_cell_peak_ptc_xyz,1), eigenspace(region_current_cell_peak_ptc_xyz,1)];
                else
                    region_current_cell_peak_features = inf;
                end
            ...
            if isempty(Region(i).cell)
                count = 1;
            else
                count = size(vertcat(Region(i).cell.id),1) + 1;
            end
            
            % Update the region
            if ~isnan(region_current_cell_peak_features) & ~isinf(region_current_cell_peak_features)
                Region(i).status = 1;
                Region(i).cell(count).id = region_current_cell_id;
                Region(i).cell(count).ptc_ids = region_current_cell_peak_ptc_ids;
                Region(i).cell(count).surface_features = region_current_cell_peak_features;
            end
            ...
            if isempty(in_region_cell_info)
                mask = ismember(Cells.cell_ids(:, [1,2]), region_current_cell_id, 'rows');
                region_local_surface_features = Cells.peak_info(mask).peaks_features(1:end-1);
            else
                % Searching interior cells
                
                region_neighbour_cell_peak_ptc_ids = region_current_cell_peak_ptc_ids;
                for k = 1:numel(in_region_cell_info(:,1))
                    ...
                    if numel(neighbour_cell_ids) ~= 8
                        ...
                        % cell
                        mask = ismember(Cells.cell_ids(:,[1,2]),neighbour_cell_peak_ids, 'rows');
                        neighbour_cell_peak_ptc_ids = vertcat(Cells.peak_info(mask).ptc_ids);
            ...
            dist_region_ptc = zeros(numel(region_current_cell_peak_ptc_ids),numel(out_region_ids) + 1);

            % For the local surface of the current region
            dist_region_ptc(:,1) = abs(dist_3Dpoints_3Dplane(region_current_cell_peak_ptc_xyz, [region_local_surface_features(1:3), region_local_surface_features(4:6)]));
            ...
            for k = 1:numel(out_region_ids)
                ...
                mask = region_info(:,end) == current_out_region_id;
                current_out_region_cell_info = region_info(mask,:);
                out_region_neighbour_cell_ids = Query_Neighbour_27Cells(TREE, current_out_region_cell_peak_ids(:,1),current_out_region_cell_info(:,1));
                % Retrieve cell ids + peak ids
                mask = ismember(current_out_region_cell_peak_ids(:,1), out_region_neighbour_cell_ids);
                out_region_neighbour_cell_peak_ids = current_out_region_cell_peak_ids(mask,[1,2]);
                
                % Compute features of the local surface of out_region and
                % distances from the points to the surface
                mask = ismember(Cells.cell_ids(:,[1,2]),out_region_neighbour_cell_peak_ids, 'rows');
                out_region_neighbour_cell_peak_ptc_ids = vertcat(Cells.peak_info(mask).ptc_ids);
                ...
                dist_region_ptc(:,k+1) = abs(dist_3Dpoints_3Dplane(region_current_cell_peak_ptc_xyz, [mean(out_region_neighbour_cell_peak_ptc_xyz,1), local_surface_normal]));
            ...
            mask_1 = any((dist_region_ptc <= THRESHOLD.dev_plane),2);
            % Condition 2: min distance
            [~, mask_2] = min(dist_region_ptc,[],2);
            mask = mask_1&mask_2;
            
            add_region_ptc_info = [region_current_cell_peak_ptc_ids(mask), region_current_cell_peak_ptc_xyz(mask,1:3), all_region_ids(mask_2(mask_1))];
            ...
            for k = 1:numel(add_region_ids)
                ...
                if numel(add_region_cell_ptc_ids) >= THRESHOLD.min_num_pts
                    ...
                    if isempty(Region(add_region_id).cell)
                        count = 1;
                    else
                        count = size(vertcat(Region(add_region_id).cell.id),1) + 1;
                    end
                    % Update region
                    add_region_cell_normal = eigenspace(add_region_cell_ptc_xyz, 1);
                    Region(add_region_id).status = 1;
                    Region(add_region_id).cell(count).id = region_current_cell_id;
                    Region(add_region_id).cell(count).ptc_ids = add_region_cell_ptc_ids;
                    Region(add_region_id).cell(count).surface_features = [mean(add_region_cell_ptc_xyz,1), add_region_cell_normal];
                end
